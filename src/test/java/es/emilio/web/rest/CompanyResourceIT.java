...
public class CompanyResourceIT {
    ...
    @Test
    @Transactional
    public void getAllCompaniesByUserExtraIsEqualToSomething() throws Exception {
        // Initialize the database
        companyRepository.saveAndFlush(company);
        UserExtra userExtra = UserExtraResourceIT.createEntity(em);
        em.persist(userExtra);
        em.flush();
        company.addUserExtra(userExtra);
        companyRepository.saveAndFlush(company);
        Long userExtraId = userExtra.getId();

        // Get all the companyList where userExtra equals to userExtraId
        defaultCompanyShouldBeFound("userExtraId.equals=" + userExtraId);

        // Get all the companyList where userExtra equals to userExtraId + 1
        defaultCompanyShouldNotBeFound("userExtraId.equals=" + (userExtraId + 1));
    }


    @Test
    @Transactional
    public void getAllCompaniesByCalendarYearUserIsEqualToSomething() throws Exception {
        // Initialize the database
        companyRepository.saveAndFlush(company);
        CalendarYearUser calendarYearUser = CalendarYearUserResourceIT.createEntity(em);
        em.persist(calendarYearUser);
        em.flush();
        company.addCalendarYearUser(calendarYearUser);
        companyRepository.saveAndFlush(company);
        Long calendarYearUserId = calendarYearUser.getId();

        // Get all the companyList where calendarYearUser equals to calendarYearUserId
        defaultCompanyShouldBeFound("calendarYearUserId.equals=" + calendarYearUserId);

        // Get all the companyList where calendarYearUser equals to calendarYearUserId + 1
        defaultCompanyShouldNotBeFound("calendarYearUserId.equals=" + (calendarYearUserId + 1));
    }
    ...
    @Test
    @Transactional
    public void getAllCompaniesByTimeBandAvailableUserDayIsEqualToSomething() throws Exception {
        // Initialize the database
        companyRepository.saveAndFlush(company);
        TimeBandAvailableUserDay timeBandAvailableUserDay = TimeBandAvailableUserDayResourceIT.createEntity(em);
        em.persist(timeBandAvailableUserDay);
        em.flush();
        company.addTimeBandAvailableUserDay(timeBandAvailableUserDay);
        companyRepository.saveAndFlush(company);
        Long timeBandAvailableUserDayId = timeBandAvailableUserDay.getId();

        // Get all the companyList where timeBandAvailableUserDay equals to timeBandAvailableUserDayId
        defaultCompanyShouldBeFound("timeBandAvailableUserDayId.equals=" + timeBandAvailableUserDayId);

        // Get all the companyList where timeBandAvailableUserDay equals to timeBandAvailableUserDayId + 1
        defaultCompanyShouldNotBeFound("timeBandAvailableUserDayId.equals=" + (timeBandAvailableUserDayId + 1));
    }
