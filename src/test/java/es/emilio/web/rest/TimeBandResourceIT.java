...
public class TimeBandResourceIT {
    ...
    @Test
    @Transactional
    public void getAllTimeBandsByCalendarYearUserIsEqualToSomething() throws Exception {
        // Initialize the database
        timeBandRepository.saveAndFlush(timeBand);
        CalendarYearUser calendarYearUser = CalendarYearUserResourceIT.createEntity(em);
        em.persist(calendarYearUser);
        em.flush();
        timeBand.addCalendarYearUser(calendarYearUser);
        timeBandRepository.saveAndFlush(timeBand);
        Long calendarYearUserId = calendarYearUser.getId();

        // Get all the timeBandList where calendarYearUser equals to calendarYearUserId
        defaultTimeBandShouldBeFound("calendarYearUserId.equals=" + calendarYearUserId);

        // Get all the timeBandList where calendarYearUser equals to calendarYearUserId + 1
        defaultTimeBandShouldNotBeFound("calendarYearUserId.equals=" + (calendarYearUserId + 1));
    }
